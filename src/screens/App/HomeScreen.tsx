...
const HomeScreen = () => {
  ...
  const updateIpAddress = () => {
    ...
    APIService.getIpAddress(localStorage.getItem("deviceToken")!)
      .then((response: any) => {
  ...
  const updateSelectedNode = () => {
    ...
    APIService.getKey("selectedNode")
      .then((response: any) => {
  ...
  const renewSubscription = () => {
    ...
    APIService.subscribeToPlan(6, payload)
      .then((response: any) => {
        connect();
      })
      .catch((e: Error) => {
  ...
  const connect = () => {
    ...
    const fetchCredentials = (sessionId: number, subscriptionId: number) => {
      ...
      APIService.fetchCredentials(payload)
        .then((response: any) => {
    ...
    const createSession = (subscriptionId: number, activeSession?: number) => {
      ...
      APIService.createSession(walletAddress, payload)
        .then((response: any) => {
    ...
    const checkSession = (subscriptionId: number, secondTime?: boolean) => {
      setLoading(
        secondTime ? "Verifying session..." : "Looking for active sessions..."
      );

      APIService.getSession(walletAddress)
        .then((response: any) => {
          if (
  ...
  return (
    <div className="homeScreenContainer">
      ...
      <div
        className={
          loading == "" ? "loadingScreen hidden" : "loadingScreen blocking"
        }
      >
        <LoadingIndicator />
        <span>{loading}</span>
      ...
      <div
        className={
          isAskingForSubscription
            ? "subscribePopUpWrapper"
            : "subscribePopUpWrapper hidden"
        }
      >
        <div className="subscribePopUp">
          ...
          <button className="button primary" onClick={renewSubscription}>
            Renew for {planPrice} DVPN
          </button>
          <button
        ...
        <div className="connectionContainer">
          <div className="selectedNodeContainer">
            {selectedNode.countryCode ? (
              <div className="selectedNodeLocation">
                ...
                <span>
                  {selectedNode.cityName + ", " + selectedNode.countryCode}
                </span>
              </div>
            ) : (
              <div className="selectedNodeLocation">
          ...
          {isConnected ? (
            <div className="actions">
              ...
              <button className="button primary" onClick={switchNode}>
                Switch Node
              </button>
            </div>
          ) : (
            <div className="actions">
              <QuickConnect
                title={"Quick Connect"}
                onClick={connect}
                disabled={selectedNode.countryCode == ""}
              />
            </div>
          )}
        </div>
