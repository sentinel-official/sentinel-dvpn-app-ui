...
const NodesCitiesScreen = () => {
  ...
  const updateServers = () => {
    APIService.getServers(
      ...
      .then((response: any) => {
        ...
        setFilteredServers(response.data.data);
      })
      .catch((e: Error) => {
  ...
  const handleOnSearchTextChange = (value: string) => {
    setSearchText(value);
    const filtered = servers.filter(({ name, address }) => {
      return (
        String(name).toLowerCase().includes(String(value).toLowerCase()) ||
        String(address).toLowerCase().includes(String(value).toLowerCase())
      );
    });
  ...
  return (
    <div className="nodesScreenContainer">
      ...
      <div className="list">
        {filteredServers.map((server: APIServer, index) => {
          return (
            <button
              id={"server-" + index}
              key={"server-" + index}
